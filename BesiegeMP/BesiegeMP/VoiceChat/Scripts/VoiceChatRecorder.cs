...
namespace VoiceChat
{
    public class VoiceChatRecorder : MonoBehaviour
    {
        ...
        Key toggleToTalkKey = spaar.ModLoader.Keybindings.AddKeybinding("Toggle To Talk", new Key(KeyCode.None, KeyCode.O));

        Key pushToTalkKey = spaar.ModLoader.Keybindings.AddKeybinding("Push To Talk", new Key(KeyCode.None, KeyCode.P));

        Key autoDetectKey = Keybindings.AddKeybinding("Auto Detect Talking", new Key(KeyCode.None, KeyCode.G));
        ...
        {
            get { return transmitToggled || forceTransmit > 0 || pushToTalkKey.IsDown(); }
        }
        ...
        void Update()
        {
            if (autoDetectKey.Pressed())
            {
                AutoDetectSpeech = !AutoDetectSpeech;
            }
            if (!recording)
            {
                return;
            ...
            if (toggleToTalkKey.Pressed())
            {
                transmitToggled = !transmitToggled;
            }

            bool transmit = transmitToggled || pushToTalkKey.IsDown();
